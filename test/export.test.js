...
async function run(authorized) {
  ...
  vm.runInNewContext(source, {app:{get:(_path,fn)=>{handler=fn;}},csvCell,
    ...
    db:{query:async(sql,args)=>{
      ...
      ]:[{nickname:'Student One',summary_text:'Three facts',score:9}]};
    }}});
...
test('teacher export includes graded and ungraded answers, summaries and escaped text',async()=>{
  ...
  assert.ok(csv.includes('"Student One","Final Summary","Three facts","9","Graded"'));
});
